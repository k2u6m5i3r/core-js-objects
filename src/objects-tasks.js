...
function shallowCopy(obj) {
  ...
  return Object.assign(Object.create(Object.prototype), obj);
}
...
function mergeObjects(objects) {
  ...
  const [itemOne, itemTwo] = objects;
  const ans = { ...itemOne, ...itemTwo };
  const objectToArrat = Object.entries(ans);
  objectToArrat.forEach((element) => {
    ans[element[0]] =
      (itemOne[element[0]] ? itemOne[element[0]] : 0) +
      (itemTwo[element[0]] ? itemTwo[element[0]] : 0);
  });
  return ans;
}
...
function makeWord(lettersObject) {
  // throw new Error('Not implemented')
  const ans = [];
  const objectToArrat = Object.entries(lettersObject);
  objectToArrat.forEach((element) => {
    const [letter, arrayIndexs] = element;
    arrayIndexs.forEach((index) => {
      ans[index] = letter;
    });
  });
  return ans.join('');
...
function sellTickets(queue) {
  ...
  if (
    ans.dol25 * 25 + ans.dol50 * 50 + ans.dol100 * 100 ===
    queue.length * 25
  ) {
    return true;
  }
...
function fromJSON(proto, json) {
  // throw new Error('Not implemented');
  const savePrototype = Object.create(proto);
  return Object.assign(savePrototype, JSON.parse(json));
}
  ...
  return new Map(
    Object.entries(
      array.reduce((acc, item) => {
        if (acc[keySelector(item)] === undefined) {
